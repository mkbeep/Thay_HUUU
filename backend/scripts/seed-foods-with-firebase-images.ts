/**
 * Script seed 50 món ăn với ảnh từ Firebase Storage
 * Chạy SAU KHI đã upload ảnh: npm run seed:foods-firebase
 */
...
/**
 * Lấy public URL từ Firebase Storage
 */
function getStorageUrl(storagePath: string): string {
  return `https://storage.googleapis.com/${bucket.name}/${storagePath}`;
}

// 50 món ăn với đường dẫn ảnh Firebase Storage
const foods = [
  // KHAI VỊ (10 món)
  {
  ...
  // ĐẶC BIỆT (10 món) - Không có ảnh trong assets, dùng placeholder
  {
    ...
    is_vegetarian: false,
    is_spicy: false,
    image_path: 'menu/specials/set-lau-hai-san.jpg',
  },
  {
  ...
  },
  {
    ...
    is_vegetarian: false,
    is_spicy: false,
    image_path: 'menu/specials/ba-wagyu.jpg',
  },
  {
    name: 'Tôm Alaska hấp',
...
async function seedFoods() {
  try {
    console.log('🌱 Bắt đầu seed 50 món ăn với ảnh Firebase Storage...\n');
    ...
    for (const food of foods) {
      ...
      // Thêm hình ảnh với Firebase Storage URL
      const imageUrl = getStorageUrl(image_path);
    ...
    console.log('\n🎉 Hoàn thành! Đã seed 50 món ăn với ảnh Firebase Storage.');
    console.log('\n📊 Thống kê:');
